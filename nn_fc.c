...
void nn_fc_init(nn_fc *fc, int8_t *weight, const int32_t *bias, size_t d_in,
    ...
    fc->out_zp = out_zp;
    nn_mtx_init(weight_mtx, d_out, d_in, weight, weight_zp);
}
...
void nn_fc_forward(const nn_fc *fc, const nn_matrix *in, nn_matrix *out) {
    ...
    for (size_t o = 0; o < fc->d_out; o++) {
        ...
        for (size_t i = 0; i < fc->d_in; i++) {
            const int32_t x = nn_q8_asym_to_int(*nn_mtx_get(in, 0, i), in->zp);
            const int32_t w =
                nn_q8_sym_to_int(*nn_mtx_get(fc->weight, o, i), fc->weight_zp);
            acc += x * w;
        }
        nn_mtx_set(out, 0, o, nn_int_to_q8_asym(acc, fc->out_zp));
    }
}
