...
void nn_conv2d_init(nn_conv2d *restrict conv2d, int8_t *weight, nn_matrix *mtx,
    ...
    for (size_t i = 0; i < conv2d->c_in * conv2d->c_out; i++) {
        nn_matrix *mtx = conv2d->mtx + i;
        int8_t *p = weight + i * h * w;
        nn_mtx_init(mtx, h, w, p, weight_zp);
    }
    conv2d->mtx = mtx;
    conv2d->c_in = c_in;
    conv2d->c_out = c_out;
...
static inline int32_t do_conv(const nn_matrix *conv, const nn_matrix *image,
    ...
    for (size_t i = 0; i < conv->h; i++) {
        for (size_t j = 0; j < conv->w; j++) {
            int conv_value = nn_to_int(*nn_mtx_get(conv, i, j), conv->zp);
            int image_value =
                nn_to_int(*nn_mtx_get(image, si + i, sj + j), image->zp);
    ...
    for (size_t i = 0; i < conv->h; i++) {
        for (size_t j = 0; j < conv->w; j++) {
            ...
            if (in_i >= 0 && in_j >= 0 && (size_t)in_i < image->h &&
                (size_t)in_j < image->w) {
                image_value = nn_to_int(
                    *nn_mtx_get(image, (size_t)in_i, (size_t)in_j), image->zp);
            }
            const int conv_value = nn_to_int(*nn_mtx_get(conv, i, j), conv->zp);
            res += conv_value * image_value;
        }
    }
    return res;
    ...
    for (size_t co = 0; co < conv2d->c_out; co++) {
        ...
        for (size_t i = 0; i < h_out; i++) {
            for (size_t j = 0; j < w_out; j++) {
                ...
                nn_mtx_set(out_mtx, i, j, nn_to_u8(acc, conv2d->out_zp));
            }
        }
    }
}
